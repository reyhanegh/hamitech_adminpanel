...
$list_url = "comment.php";
// ----------- urls ----------------------------------------------------------------------------------------------------

// ----------- get data ------------------------------------------------------------------------------------------------
$counter = 1;
$result = $action->post_list('comment');
// ----------- get data ------------------------------------------------------------------------------------------------

// ----------- delete --------------------------------------------------------------------------------------------------
if (isset($_GET['remove'])) {
    $id = $action->request('remove');
    $_SESSION['error'] = !$action->post_remove($id,'comment');
...
if (isset($_GET['status'])) {
    ...
    $_SESSION['error'] = !$action->comment_status($id);
    echo $_SESSION['error'];
    header("Location: $list_url");
    return ;
}
                            ...
                            <div class="table-responsive m-t-5">
                                <table id="example23" class="display nowrap table table-hover table-striped"
                                    ...
                                    <th class="text-center">ردیف</th>
                                        ...
                                        <th class="text-center">نام کاربری نظر دهنده</th>
                                    ...
                                    <? while ($row = $result->fetch_object()) { ?>
                                        <tr class="text-center">
                                            ...
                                            <td class="text-center"><?= $action->get_data('tbl_post',$row->post_id)->title?></td>
                                            <td class="text-center"><?= $row->name ?></td>
                                            <td class="text-center"><? 
                                                $user=$action->get_data('tbl_user',$row->user_id);
                                                if(isset($user->username)){echo $user->username;}else{echo "کاربر مهمان";}
                                                ?></td>
                                            <td class="text-center"><?= $action->time_to_shamsi($row->date_c)?></td>
                                            <td class="text-center"><?= $action->time_to_shamsi($row->updated_at)?></td>
                                            <td class="text-center">
                                                <a href="<?= $list_url ?>?status=<?= $row->id ?>">
                                                    <?
                                                    if (($row->status)) echo "<status-indicator positive pulse></status-indicator>"." تایید شده ";
                                                    else echo "<status-indicator negative pulse></status-indicator>"." تایید نشده  ";
                                                    ?>
                                                </a>
                                            </td>
